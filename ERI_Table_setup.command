...
if [ $testname == "Airport" ] || [ $testname == "airport" ]; then
	if [ $testid == "1" ]; then
		...
		echo "If this fails, it could mean that the PCI BUS does not read the BroadCom chip, which may mean that there is either damage to the card, or port damage on the connector."
		no_param
		...
		echo "If you are missing any of these ports then there is a good chance that caused this test failure. Otherwise, treat this like it has a failing BroadCom Chip."
		no_param
		...
		echo -e "It different from test 8 by looking for errors within the register and not just an error with the registers themselves."
		no_param
		...
		echo -e "This test can fail due to external devices being plugged in, or being used. For example, if you have a mouse/keyboard plugged in, moving the mouse or hitting any key on the keyboard will wake the system and cause a false-fail. This also applies to plugging in a monitor or USB Device.\n"
		no_param
		...
		echo
		echo "You have selected "$testname":"$testid": Chamber Performance Test\n"
...
if [ $testname == "AudioPort2" ] || [ $testname == "audioport2" ] || [ $testname == "AudioPort" ] || [ $testname == "audioport" ]; then
	if [ $testid == "2" ]; then
		...
		echo "Total Harmonic Distortion + Noise (THD+N) is a measurement of distortion present within a signal. If this test fails, you could have a few issues at hand; improper electromagnetic interference (EMI) setup, bad filtering or grounds, or a bad transmitter or receiver." 
		...
		echo "This test requires that you have an ADA Box plugged in an that you use the Analog cable."
		echo "Signal-to-Noise Ratio (SNR) is "
		no_param
...
if [ $testname == "EthernetPort" ] || [ $testname == "ethernetport" ]; then
	if [ $testid == "4" ]; then
		...
		echo "This test requires a requires a loopback to be connected between ETH0 and ETH1 before running."
		no_param
